...
const sandboxStyles = ref('')  // 当前加载的组件 CSS
...
/** 解析 .vue SFC 代码，提取 template + script + style 合并为组件定义 */
function parseSFC(code: string): { componentDef: Record<string, any> | null; css: string } | null {
  // 提取 <template> 内容
...
function previewPastedCode() {
  ...
  try {
    const result = parseSFC(pastedCode.value)
    if (result && result.componentDef) {
      currentComponentDef.value = result.componentDef as any
      sandboxStyles.value = result.css
      selectedDemo.value = ''
